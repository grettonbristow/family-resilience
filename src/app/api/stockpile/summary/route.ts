...
export async function GET() {
  try {
    ...
    const items = await db.select().from(stockpileItems).where(eq(stockpileItems.userId, userId));

    // Food: sum calories / (household x 2000 cal/day)
    ...
    // Water: sum liters / (household x 3 liters/day)
